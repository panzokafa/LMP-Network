            ...
            <div class="inter  lg:w-1/2">
                ...
                <div class="flex lg:flex-row flex-col lg:gap-7 gap-4  items-center">
                    <div
                        ...
                        Pre
                        Order</div>
                    <a href="{{ url('brosur/' . $product->brosur) }}" target='_blank'
            ...
            <div class="grid lg:grid-cols-2 grid-cols-1 gap-7 ">
                ...
                @foreach ($similar as $product)
                    <a href="{{ route('user.product', $product->id) }}"
                        class="{{ Route::current()->parameter('product_id') == $product->id ? 'hidden' : 'block' }} flex hover:border-[#A8D5FB] border-4 border-white duration-300 justify-center items-center bg-[#DDEDFB] px-6 py-8 cursor-pointer h-full">
                        <div>
                            <div class="lg:text-xl sm:text-lg  mb-5 font-medium">
                                {{ $product->name }}
                            </div>

                            <div class="lg:text-base sm:text-base text-xs line-clamp-2">
